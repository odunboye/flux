...
encode (JArray xs)   = "[" ++ commas (map encode xs) ++ "]"
  where
    commas : List String -> String
    commas [] = ""
    commas [x] = x
    commas (x :: xs) = x ++ ", " ++ commas xs
encode (JObject kvs) = "{" ++ commas (map encodeKV (toList kvs)) ++ "}"
  where
    commas : List String -> String
    commas [] = ""
    commas [x] = x
...
jsonError code msg =
  let bodyStr = encode (JObject (fromList [("error", JString msg)]))
      body = fromString bodyStr
      hs = [("Content-Type", "application/json"), ("Content-Length", show (length bodyStr))]
